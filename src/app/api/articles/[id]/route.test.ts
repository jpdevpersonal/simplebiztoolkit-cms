...
describe("/api/articles/[id]", () => {
  ...
  it("PUT returns unauthorized when auth fails", async () => {
    ...
    const request = new Request("http://localhost/api/articles/42", {
      method: "PUT",
  ...
  it("GET proxies article request when no auth available", async () => {
    ...
    // Simulate requireAuth returning not-ok so GET will proceed without access token
    requireAuthMock.mockResolvedValue({
      ok: false,
